...
export async function reviewWellness(reviewer: Reviewer, args: {
  ...
  const { result: review, hash } = await reviewer.reviewWellnessSignal(bundle);
  events = pushEvent(events, makeEvent("GENLAYER_REVIEW_COMPLETED", `risk level: ${review.riskLevel}`, { tone: review.riskLevel === "HIGH" ? "danger" : review.riskLevel === "ELEVATED" ? "warn" : "ok", txHash: hash }));
  ...
  return { bundle, review, hash, events };
}
...
export async function reviewGoal(reviewer: Reviewer, input: {
  ...
  const { result: review, hash } = await reviewer.reviewGoalAccountability({ ...rest, rawJournalIncluded: false });
  events = pushEvent(events, makeEvent("GOAL_REVIEW_COMPLETED", `outcome: ${review.outcome}`, { tone: review.outcome === "COMPLETED" ? "ok" : review.outcome === "MISSED" ? "warn" : "info", txHash: hash }));
  return { review, hash, events };
}
...
export async function reviewReply(reviewer: Reviewer, input: { userHash: string; circleAlias: string; replyText: string; events: EunoiaEvent[]; }) {
  ...
  const { result: review, hash } = await reviewer.reviewSupportReply({ userHash: input.userHash, circleAlias: input.circleAlias, replyText: input.replyText, contextType: "support_circle" });
  events = pushEvent(events, makeEvent("SUPPORT_REPLY_REVIEWED", `classification: ${review.classification}`, { tone: review.visible ? "ok" : "warn", txHash: hash }));
...
export async function reviewConsent(reviewer: Reviewer, input: {
  ...
  const { result: review, hash } = await reviewer.reviewResearchConsent({ title: input.title, requestingEntity: input.requestingEntity, dataRequested: input.dataRequested, dataNotRequested: input.dataNotRequested, durationDays: input.durationDays, revocable: input.revocable });
  events = pushEvent(events, makeEvent("CONSENT_REVIEW_COMPLETED", `status: ${review.status}`, { tone: review.status === "CLEAR" ? "ok" : review.status === "REJECTED" ? "danger" : "warn", txHash: hash }));
  return { review, hash, events };
}
...
export async function reviewCheckin(reviewer: Reviewer, input: { userHash: string; riskLevel: string; recentMoodAvg: number; recentStressAvg: number; missedGoals: number; preferredTone: string; events: EunoiaEvent[]; }) {
  ...
  const { result: review, hash } = await reviewer.reviewCheckinTrigger({
  ...
  if (review.shouldTrigger) events = pushEvent(events, makeEvent("CHECKIN_TRIGGERED", `tone: ${review.tone}`, { tone: "info", txHash: hash }));
  return { review, hash, events };
}
